...
public class HTTPMonitoringInterceptor implements HandlerInterceptor {
    ...
    public void postHandle(HttpServletRequest httpServletRequest, HttpServletResponse
        ...
        // Metrics moved to afterCompletion to avoid calling getMatchingURLPattern here,
        // which can throw when request attributes (e.g. lookupPath) are not set.
    }
    ...
    public void afterCompletion(HttpServletRequest httpServletRequest, HttpServletResponse
        ...
        if (!path.startsWith("/carts") && !path.equals("/health")) {
            return;
        }
    ...
    /** Brave MDC keys; embed in message so filelog/Quickwit can parse even if layout differs. */
